...
class RecipeView extends View {
  ...
  _errMess = 'We could not fetch your recipe. Please try again!';
  _mess = '';
  ...
  servingsHandler(handler) {
    this._parentEl.addEventListener('click', function (e) {
      ...
      if (!btn) return;
      const serving = +btn.dataset.update;
      if (serving <= 0) return;
      handler(serving);
    });
  }

  addHandlerAddBookMarkup(handler) {
    this._parentEl.addEventListener('click', function (e) {
      const mark = e.target.closest('.btn--bookmark');
      if (!mark) return;
      handler();
    });
  }
